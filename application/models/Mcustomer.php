...
class Mcustomer extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT max(idpesanan)+1 as nopesanan from tblpesanan");
		return $query->result();
	}
	...
	{
		$query = $this->db->query("UPDATE tblpesanan SET statuspesanan = 'Pesanan Di Terima'  where idpesanan = '" . $id . "'");
		return true;
	}

	public function cekdatapesanan($kodepesanan)
	{
		$q = $this->db->query("select a.*,b.idproduk,c.idproduk as id_produk ,c.namaproduk,c.gambar from tblpesanan a left join tblposting b on a.idpostingproduk = b.idposting left join tblproduk c on b.idproduk = c.idproduk where a.nohp = '" . $kodepesanan . "'");
		return $q->result();
	}
